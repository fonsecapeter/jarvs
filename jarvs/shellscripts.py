...
class Scripts:
	def __init__(self):
		...
		'rvsoverduecount="0"',
		'',
		'echo "Checking files that belong to ${name_first}..."',
		'#if [ $OS == "Linux" ] 2> /dev/null; then',
		'DATE=$(date +%Y%m%d)',
		'datedash=$(date +%Y-%m-%d)',
		'#else',
		...
		'if [[ $(wc -l <${HOME}/.jarvs/RVS_report.csv) -ge 1440 ]]; then',
		'  mkdir -p "${HOME}/.jarvs/old_reports"',
		'  mv "${HOME}/.jarvs/RVS_report.csv" "${HOME}/.jarvs/old_reports/"',
		'  mv "${HOME}/.jarvs/old_reports/RVS_report.csv" "${HOME}/.jarvs/old_reports/RVS_report_${datedash}.csv"',
		...
		'##echo "> $name_first RVSs reported on [$datedash]"',
		'}',
		'',
		'',
		...
		'done',
		'',
		'echo "RVSs reported on $(date +%Y%m%d)"',
